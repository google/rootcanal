...
class Test(ControllerTest):
    ...
    async def test_peripheral_accepted(self):
        ...
        controller.send_cmd(
            hci.LeAcceptCisRequest(connection_handle=cis_connection_handle)
        )

        # The IUT sends a successful Command Status to the Upper Tester.
        await self.expect_evt(
            hci.LeAcceptCisRequestStatus(
        ...
        cis_rsp = await self.expect_llcp(
            source_address=controller.address,
            destination_address=peer_address,
            expected_pdu=llcp.CisRsp(
                cis_offset_min=self.Any, cis_offset_max=self.Any, conn_event_count=0
            ),
        )
        ...
        await self.expect_evt(
            hci.LeCisEstablishedV1(
                status=hci.ErrorCode.REMOTE_USER_TERMINATED_CONNECTION,
    ...
    async def test_central_initiating(self):
        ...
        cis_req = await self.expect_llcp(
            source_address=controller.address,
            destination_address=peer_address,
    ...
    async def test_central_connected(self):
        ...
        controller.send_llcp(
            ...
            destination_address=controller.address,
            pdu=llcp.CisRsp(
                cis_offset_min=cis_req.cis_offset_min,
        ...
        cis_ind = await self.expect_llcp(
            source_address=controller.address,
            destination_address=peer_address,
