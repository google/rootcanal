    ...
    async def test(self):
        ...
        await self.expect_ll(
            ll.RoleSwitchRequest(
                source_address=controller.address, destination_address=peer_address
            )
        )
        ...
        controller.send_cmd(
            hci.SwitchRole(bd_addr=peer_address, role=hci.Role.PERIPHERAL)
        )
        ...
        await self.expect_evt(
            hci.RoleChange(
                status=ErrorCode.ROLE_SWITCH_FAILED,
                bd_addr=peer_address,
                new_role=hci.Role.PERIPHERAL,
            )
        )
